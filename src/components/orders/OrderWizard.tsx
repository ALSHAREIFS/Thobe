                ...
                <div>
                  ...
                  {isEditingMode ? (
                    <div className="w-full px-3 py-2 text-base font-black bg-stone-100 rounded-xl border border-stone-300 text-emerald-800 text-center flex items-center justify-center">
                      {initialEditingOrder?.pricing?.paidAmount || 0} {getCurrencySymbol(currentShop?.currency)}
                    </div>
                  ) : (
                    <input
                      type="text" inputMode="decimal"
                ...
                {vatConfig.vatEnabled && ((currentShop?.country || 'SA') === 'SA' && currentShop?.currency === 'SAR') && (
                  <>
                    <div>
                      ...
                      <div className="text-base font-black text-slate-700 mt-0.5">{formatCurrency(subtotalAmount, currentShop?.currency)}</div>
                    </div>
                    <div>
                      <div className="text-xs text-slate-500 font-semibold">ضريبة القيمة المضافة ({vatConfig.vatRate}%)</div>
                      <div className="text-base font-black text-blue-800 mt-0.5">{formatCurrency(vatAmount, currentShop?.currency)}</div>
                    </div>
                  </>
                )}
                <div>
                  <div className="text-xs text-slate-500 font-semibold">إجمالي الطلب ({quantity} ثياب)</div>
                  <div className="text-lg font-black text-slate-900 mt-0.5">{formatCurrency(totalAmount, currentShop?.currency)}</div>
                </div>
                <div>
                  <div className="text-xs text-emerald-700 font-semibold">المدفوع (العربون)</div>
                  <div className="text-lg font-black text-emerald-700 mt-0.5">{paidAmount} {getCurrencySymbol(currentShop?.currency)}</div>
                </div>
                <div>
                  <div className="text-xs text-[#1A365D] font-semibold">المتبقي عند الاستلام</div>
                  <div className="text-lg font-black text-[#1A365D] mt-0.5">{remainingAmount} {getCurrencySymbol(currentShop?.currency)}</div>
                </div>
              </div>
            ...
            <div className="p-4 bg-blue-50/60 rounded-2xl border border-blue-200 flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-6 text-center sm:text-right flex-wrap">
                {vatConfig.vatEnabled && ((currentShop?.country || 'SA') === 'SA' && currentShop?.currency === 'SAR') && (
                  <>
                    <div>
                      ...
                      <span className="font-bold text-base text-slate-700">{formatCurrency(subtotalAmount, currentShop?.currency)}</span>
                    </div>
                    <div>
                      <span className="text-xs text-blue-800 block">ضريبة ({vatConfig.vatRate}%):</span>
                      <span className="font-bold text-base text-blue-900">{formatCurrency(vatAmount, currentShop?.currency)}</span>
                    </div>
                  </>
                )}
                <div>
                  ...
                  <span className="font-black text-lg text-slate-900">{formatCurrency(totalAmount, currentShop?.currency)}</span>
                </div>
                <div>
                  <span className="text-xs text-emerald-700 block">العربون:</span>
                  <span className="font-black text-lg text-emerald-700">{paidAmount} {getCurrencySymbol(currentShop?.currency)}</span>
                  {paidAmount > 0 && (
                ...
                <div>
                  ...
                  <span className="font-black text-lg text-[#1A365D]">{remainingAmount} {getCurrencySymbol(currentShop?.currency)}</span>
                </div>
              </div>
